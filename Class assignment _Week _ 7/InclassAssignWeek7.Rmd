...
summary(elections08)
class(elections08$day)
DaystoElections08 <- as.Date ("2008-11-04") - as.Date (elections08$day)
range(DaystoElections08)
n <- length(elections08$day)
Obama.pred <- McCain.pred <- rep(NA, length(n))
for (i in 1:n) {
  market_price2008 <- subset(elections08, subset = ((DaystoElections08 <= ( n -i)) & (DaystoElections08 > (n - i))))
}
Obama.pred [i] <- mean(elections08$PriceD)
McCain.pred [i] <- mean(elections08$PriceR)
